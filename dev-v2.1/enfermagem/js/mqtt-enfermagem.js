...
let client_name = 'cetam';
//var my_id = "enf-3";
let my_id = getCookie("my_id");

let port = 9001;

let topic = client_name + '/enfermagem/' + my_id + '/#';
...
function MQTTconnect() {
    ...
    mqtt = new Paho.MQTT.Client(host, port, path, my_id);
    let options = {
        timeout: 3,
        useSSL: useTLS,
...
function onConnect() {
    ...
    if(my_id != ""){
      ...
      connect_topic = client_name + "/enfermagem/" + my_id + "/connect";
...
function onMessageArrived(message) {
    ...
    if(comando == "start"){
      ...
      sendMessage("cetam/enfermagem/" + my_id + "/connect", "online");
      if(my_id != ""){
    ...
    if(comando == "reload"){
      sendMessage("cetam/enfermagem/" + my_id + "/connect", "offline");
      location.reload();
    }      
    if(comando == "close"){
      window.close();

...
function onMessageArrived(message) {
  ...
  if(comando == "connect"){
    mac_address = id; // pega o mac address de quem enviou o connect
    connected_audio = document.getElementById("endpoint_" + mac_address);
    if(payload == 'online'){
...
function initEnfermagem(id) {
  if (id == "b827ebd97628") {
    session_id = "2_MX40NzE3MzczNH5-MTYzNjA1MTk2NTc5MH5HWVdJS1V5Tm45SFRncXlmLy9hRGpWcWx-UH4";
    apiKey = "47173734";
    token = "T1==";
...
function micSwitch(client, type, id){
  console.log(id);
  micIntercom_icon = document.getElementById("micIntercom_" + id);
  topic_intercom = client + '/' + type + '/' + id + '/audio';
  topic_commandcenter = 'cetam/commandcenter/cc-1/audio';
  
  if(micIntercom_icon.classList.value == "bi bi-mic-mute-fill"){
    console.log("Abrindo speaker do intercom");
    micIntercom_icon.classList.value = "bi bi-mic-fill";
    micIntercom_icon.style.color = "blue";
    command_intercom = {'type' : 'speaker', 'command' : 'on', 'message' : 'ligar'};
    console.log("Abrindo microfone do local");
    command_commandcenter = {'type' : 'mic', 'command' : 'on', 'message' : 'intercom'};

  }else{
    console.log("Fechando speaker do intercom");
    micIntercom_icon.classList.value = "bi bi-mic-mute-fill";
    micIntercom_icon.style.color = "red";
    command_intercom = {'type' : 'speaker', 'command' : 'off', 'message' : 'desligar'};
    console.log("Fechando microfone local");
    command_commandcenter = {'type' : 'mic', 'command' : 'off', 'message' : 'intercom'};
  }
  sendJSONMessage(topic_intercom, command_intercom);
...
function speakerSwitch(client, type, id){
  console.log(id);
  speakerIntercom_icon = document.getElementById("speakerIntercom_" + id);
  topic_intercom = client + '/' + type + '/' + id + '/audio';
  topic_commandcenter = 'cetam/commandcenter/cc-1/audio';

  if(speakerIntercom_icon.style.color == "red"){
    console.log("Abrindo microfone do intercom");
    speakerIntercom_icon.style.color = "blue";
    command_intercom = {'type' : 'mic', 'command' : 'on', 'message' : 'ligar'};
    console.log("Abrindo speaker local");
    command_commandcenter = {'type' : 'speaker', 'command' : 'on', 'message' : 'intercom'};
  }else{
    console.log("Microfone do intercom sempre aberto.");
    speakerIntercom_icon.style.color = "red";
    command_intercom = {'type' : 'mic', 'command' : 'on', 'message' : 'desligar'}; // microfone SEMPRE ABERTO no intercom
    console.log("Fechando speaker local");
    command_commandcenter = {'type' : 'speaker', 'command' : 'off', 'message' : 'intercom'};
  }
  sendJSONMessage(topic_intercom, command_intercom);
  sendJSONMessage(topic_commandcenter, command_commandcenter);
}

...
function initializeSession(key, session_id, my_token) {
  ...
  publisher_commandcenter.on("streamDestroyed", function (event) {
    console.log("The publisher stopped streaming. Reason: "
      + event.reason);
    sendMessage("cetam/commandcenter/" + my_id + "/connect", "offline");
  });

  publisher_commandcenter.on("streamCreated", function (event) {
    console.log("Stream do publisher command center criado! ");
...
function setVideo(state){
  publisher_commandcenter.publishVideo(state);
}
